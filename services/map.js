...
module.exports.draw = () => {
    ...
    cities.forEach((city) => {
        URL.concat("&markers=anchor:center%7Cicon:");
        if(city.count === 1){
            URL.concat("https://i.imgur.com/5jWQvJO.png%7C", city.name);
        } else if (city.count <= 5){
            URL.concat("https://i.imgur.com/6PTHNjg.png%7C", city.name);
        } else {
            URL.concat("https://i.imgur.com/ihbMjUv.png%7C", city.name);
        }
    });
